...
#include "Source/Shaders/Base/PushConstants.glsl"
#include "Source/Shaders/Base/TextureArrays.glsl"
#include "Source/Shaders/Base/EntityInfo.glsl"
...
float roughness = params[0][0];
float lodBias = 0.0f;//pFilterParameters.fparams[1];
uint sampleCount = 1024;//pFilterParameters.uparams[0];
//int currentMipLevel = floatBitsToInt(params[0][1]);
int width = floatBitsToInt(params[0][2]);
int distribution = floatBitsToInt(params[0][3]);
int faceoutput = floatBitsToInt(params[1][1]);
...
void writeFace(int face, vec4 color)
{
	//if(face == 0)
		outFace0 = color;
	/*else if(face == 1)
...
vec4 getImportanceSample(int sampleIndex, vec3 N, float roughness)
{
    ...
    vec2 xi = hammersley2d(sampleIndex, int(sampleCount));
...
float computeLod(float pdf)
{
    ...
    // float omegaP = 4.0 * UX3D_MATH_PI / (6.0 * float(width) * float(width));
...
vec4 filterColor(in samplerCube uCubeMap, vec3 N)
{
    ...
    for(int i = 0; i < int(sampleCount); ++i)
    {
        ...
        if(distribution == cLambertian)
        {
            ...
            vec4 lambertian = textureLod(uCubeMap, H.xyz, lod);
        ...
        else if(distribution == cGGX || distribution == cCharlie)
        {
            ...
            if (NdotL > 0.0)
            {
                ...
                lod = 0.0f;
                vec4 sampleColor = textureLod(uCubeMap, L, lod);
...
vec3 LUT(float NdotV, float roughness)
{
    ...
    return vec3(4.0 * A, 4.0 * B, 4.0 * 2.0 * UX3D_MATH_PI * C) / float(sampleCount);
}
...
void filterCube(in samplerCube tex, in int face)
{
	...
	//for(int face = 0; face < 6; ++face)
	{
		...
		writeFace(face, filterColor(tex , direction));
		//writeFace(face, textureLod(tex , direction, currentMipLevel));
...
void main() 
{
    filterCube(textureCubeSampler[floatBitsToInt(params[1][0])], faceoutput);

    //Testing:
    /*
    outFace0 = vec4(inUV,0,1);
    outFace1 = vec4(inUV,0,1);
    outFace2 = vec4(inUV,0,1);
    outFace3 = vec4(inUV,0,1);
    outFace4 = vec4(inUV,0,1);
    outFace5 = vec4(inUV,0,1);
    */
    /*
    outFace0 = vec4(1,0,0,1);
    outFace1 = vec4(1,0,0,1);
    outFace2 = vec4(0,1,0,1);
    outFace3 = vec4(0,1,0,1);
    outFace4 = vec4(0,0,1,1);
    outFace5 = vec4(0,0,1,1);
    */
